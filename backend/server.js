...
app.post('/api/reviews', async (req, res) => {
  try {
    ...
    try {
      ...
      await transporter.sendMail({
        ...
        html: `
          <div style="font-family: sans-serif; padding: 20px; border: 1px solid #ddd; max-width: 600px;">
            <h2 style="color: #0f172a; border-bottom: 2px solid #d4af37;">New Client Review Submitted</h2>
      ...
      console.log(`Express SMTP review email sent successfully for ${name}.`);
    } catch (emailErr) {
      console.error('Express SMTP Review Email Error:', emailErr);
    }

    res.json({ success: true, message: 'Review posted and email notification sent successfully.' });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
...
app.post('/api/contact', async (req, res) => {
  try {
    ...
    try {
      ...
      await transporter.sendMail({
        from: `"${name || 'KIRS Lead'}" <${process.env.SMTP_USER || 'website@kirs.co.in'}>`,
        to: process.env.SMTP_TO || 'info@kirs.co.in',
        subject: mailSubject,
        html: mailHtml,
      });
